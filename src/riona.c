...
int get_k_plus(int k_neigh, int num_train, float *dissim, int *indices)
{
    int k_plus = k_neigh;
    while(k_plus < num_train && dissim[indices[k_plus]] == dissim[indices[k_neigh - 1]])
    {
        k_plus++;
    }
    return k_plus;
...
// printf("666\n");
    // float train_values_t[num_attr][num_train];

    #ifdef VECT
    float (*train_values_t) [num_train] = malloc( sizeof(float[num_attr][num_train]) );
    cblas_somatcopy(CblasRowMajor, CblasTrans, num_train, num_attr, 1, (float *)train_values, 
		     num_attr, (float *)train_values_t, num_train);
    #endif
...
// printf("777\n");
    ...
    {
        // float dissim[num_train];
        // memset(dissim, 0, sizeof(dissim));
        ...
        calculate_dissimilairty_cuda(test_values[i], num_train, (float *)train_values, 
        num_attr, attr_types, min_values_train, max_values_train, num_unique_classes, 
        values_classes_cnt, unique_values, unique_values_cnt, num_unique_values, dissim);
        #else
        for (int j = 0; j < num_train; j++)
